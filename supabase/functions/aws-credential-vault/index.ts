...
export const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (action === "decrypt") {
      // SECURITY: decrypt is owner-scoped. Users may only decrypt their own rows.
      // Guardian-scheduler decrypts in-process and should not call this over the network.
      ...
      const accessKeyId = await decryptValue(cred.encrypted_access_key_id, user.id);
      const secretAccessKey = await decryptValue(cred.encrypted_secret_access_key, user.id);
      const sessionToken = cred.encrypted_session_token
        ? await decryptValue(cred.encrypted_session_token, user.id)
        : undefined;
    ...
    console.error("[aws-credential-vault] error:", err?.name || "Error");
    return new Response(JSON.stringify({ error: "Vault operation failed" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
